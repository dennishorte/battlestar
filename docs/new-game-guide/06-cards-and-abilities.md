...
### Auto-Generating Card Index Files

Once cards are split into one file each, do not hand-maintain a per-directory `index.js` that lists them all — those registries are pure overhead, easy to drift out of sync with the files on disk, and one of the most common sources of "I added a card but the game can't see it" bugs. Use the `regen-card-indexes` codegen instead:
...
// ...one require per file, sorted alphabetically...
...
To opt a directory in, seed its `index.js` with the marker line above and run `npm run regen-card-indexes -w common`. The script walks the workspace, finds every `index.js` carrying that marker, scans the directory for non-test `*.js` files, and rewrites the file as a sorted require list plus a `module.exports = [...]` array. Adding a new card is then: drop the file, run the script. No manual registry edits, no drift.

Why a build-time codegen instead of a runtime `fs.readdirSync` helper: card-index files are bundled into the browser via Vite, where `fs` and `__dirname` don't exist. The committed static index.js works identically in Node tests, build scripts, and the browser bundle.
...
If a game has cards organized by source-subfolder (e.g. `imperium/{base,bloodlines,uprising}/`), each subfolder gets its own auto-generated `index.js`; the parent `index.js` is a hand-written aggregator that spreads them. If cards from multiple sources live in one flat folder (Dune's `intrigue/` is the canonical example), each card declares its own `source: 'Uprising'` etc. and a single auto-generated index covers them all.

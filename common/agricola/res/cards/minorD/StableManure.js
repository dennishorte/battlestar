module.exports = {
  ...
  matches_onHarvest(_game, player) {
    return player.getUnfencedStableCount() > 0
      && player.getSownFields().some(f => !f.isVirtualField)
  },
  onHarvest(game, player) {
    ...
    if (extraHarvested > 0) {
      game.log.add({
        template: '{player} harvests {count} extra good(s)',
        args: { player, count: extraHarvested },
      })
    }
  },
}

...
| 2 | `BaseCardManager` source indexing + codegen | Engine (additive) | Low | — | 1–2 days | ✓ Completed 2026-05-11 (codegen, not runtime helper — see section) |
...
**Status.** ✓ Completed 2026-05-11 — but not as originally designed.

First attempt (`bd36b4982`) made the per-card-type `index.js` files one-line wrappers around `BaseCardManager.loadFromDirectory(__dirname)`. This broke the Vite-bundled app runtime: `fs.readdirSync` and `__dirname` don't exist in the browser, and the dev server failed with `Uncaught ReferenceError: __dirname is not defined`.

The fix is a build-time codegen rather than a runtime helper. A `regen-card-indexes` Node script (`common/scripts/regen-card-indexes.js`, npm-runnable as `npm run regen-card-indexes -w common`) walks the workspace, finds every `index.js` carrying the `// AUTO-GENERATED by ... Do not edit by hand.` marker line, and rewrites it as a sorted list of `require('./<file>.js')` calls plus a `module.exports = [...]` array. The committed file is 100% static `require()`s — works identically in Node tests, build scripts, and the browser bundle. Adding a card is: drop the file, run the script.
...
| 2 — `BaseCardManager` source indexing + codegen | 2026-05-11 | first attempt `bd36b4982`, replaced by `regen-card-indexes` codegen | First attempt used a runtime `loadFromDirectory` helper; broke the Vite app bundle (`fs.readdirSync` / `__dirname` aren't available in browsers). Replaced with a `npm run regen-card-indexes -w common` script that rewrites marker-tagged `index.js` files as static require lists. Same zero-registry-edits ergonomics, works universally. `BaseCardManager.filterDefinitions` (pure JS) stays as a runtime helper |

...
module.exports = {
  ...
  endgameEffect(game, player) {
    ...
    if (factionsAt3 >= 4) {
      player.incrementCounter('vp', 2, { silent: true, source: 'Plans Within Plans (intrigue)' })
      game.log.add({ template: '{player}: +2 VP (3+ on all 4 Factions)', args: { player } })
    }
    else if (factionsAt3 >= 3) {
      player.incrementCounter('vp', 1, { silent: true, source: 'Plans Within Plans (intrigue)' })
